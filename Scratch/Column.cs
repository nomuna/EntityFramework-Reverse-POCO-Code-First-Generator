...
namespace Scratch
{
    public class Column
    {
        ...
        private static readonly List<string> NotNullable = new List<string>
        {
            "string",
            "byte[]",
            "datatable",
        ...
        private void SetupEntity()
        {
            ...
            var initialization = Settings.UsePropertyInitializers
                ? (Default == String.Empty ? "" : String.Format(" = {0};", Default))
                : "";
            Entity = String.Format("public {0}{1} {2} {{ get; {3}set; }}{4}{5}", (OverrideModifier ? "override " : ""),
        ...
        private void SetupConfig()
        {
            ...
            if (IsIdentity || isNewSequentialId)
            {
                if (Settings.UseDataAnnotations || isNewSequentialId)
                    ...
                    databaseGeneratedOption =
                        String.Format(".HasDatabaseGeneratedOption({0}DatabaseGeneratedOption.Identity)", schemaReference);
            }
            else if (IsComputed())
            {
                if (Settings.UseDataAnnotations)
                    ...
                    databaseGeneratedOption =
                        String.Format(".HasDatabaseGeneratedOption({0}DatabaseGeneratedOption.Computed)", schemaReference);
            }
            else if (IsPrimaryKey)
            {
                if (Settings.UseDataAnnotations)
                    ...
                    databaseGeneratedOption = String.Format(".HasDatabaseGeneratedOption({0}DatabaseGeneratedOption.None)",
                        schemaReference);
            }
            ...
            if (Settings.UseDataAnnotations)
                DataAnnotations.Add(String.Format("Column(@\"{0}\", Order = {1}, TypeName = \"{2}\")", Name, Ordinal,
                    SqlPropertyType));
            else
